...
public class PactConsumerTest {
    ...
    public void testConsumerContractForGetAllUsers () {
        ...
        // Contract definition span
        RequestResponsePact responsePact;
        Span contractSpan = tracer.spanBuilder("contract").setParent(Context.current().with(parentSpan)).startSpan();
        try (Scope contractScope = contractSpan.makeCurrent()) {
            try {
        ...
        } finally { contractSpan.end(); }

        // Execution span
        PactVerificationResult pactConsumerResult;
        Span executionSpan = tracer.spanBuilder("execution").setParent(Context.current().with(parentSpan)).startSpan();
        try (Scope execScope = executionSpan.makeCurrent()) {
            try {
